...
class AdminController {
  ...
  async handlePremiumStep(chatId, userId, text) {
    ...
    const message = `✅ Premium prediction saved!\n\n` +
                   ...
                   `Type /skip to save the prediction now, or /addanalysis to add free analysis data.`;

    await this.bot.sendMessage(chatId, message, { parse_mode: 'Markdown' });
  }

  /**
   * Handle analysis step (placeholder)
   */
  async handleAnalysisStep(chatId, userId, text) {
    await this.savePrediction(chatId, userId);
  }
